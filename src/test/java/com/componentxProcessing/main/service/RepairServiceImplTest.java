...
class RepairServiceImplTest {
	...
	/*
	 * @Test public void testProcessServiceSuccess() { ProcessRequest
	 * processRequestObj = new ProcessRequest(0, "shivam", 9876543210L,
	 * "8989898989898989", true, "Integral", "mouse", 5); String token = "token";
	 * when(processResponseObj).thenReturn(new ProcessResponse());
	 * when(packagingClient.save(processRequestObj.getComponentType(),
	 * processRequestObj.getQuantity(), token)).thenReturn(new
	 * PackagingAndDeliveryDTO(25.0)); ProcessResponse processResponseObj =
	 * repairServiceImpl.processService(processRequestObj, token);
